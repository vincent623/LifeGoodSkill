...
const PROJECTFIGS: Record<string, { suffixes: string[]; relatedSearches: string[] }> = {
  "skill-repository": {
...
function loadCache(query: string): SearchResult[] | null {
  try {
    const cacheFile = `${CACHE_DIR}/${encodeURIComponent(query)}.json`;
    if (!existsSync(cacheFile)) return null;

    const cache: SearchCache = JSON.parse(readFileSync(cacheFile, "utf-8"));
...
function saveReportToFile(filename: string, data: any) {
  try {
    let content = "";
    const timestamp = new Date().toISOString().replace(/T/, " ").replace(/\..+/, "");

    if (data.existingItems && data.analysis?.keywords) {
      // 探索报告 -> Markdown
      content = `# Skill Searcher 探索报告
...
${data.projectDescription?.slice(0, 300)}...

## 现有资源

${data.existingItems?.map((i: string) => `- ${i}`).join("\n") || "无"}

## 资源分类

${data.categories?.join(", ") || "未识别"}

## 用户意图

${data.userIntent || "探索相关资源"}

## 语义分析

**核心关键词**: ${data.analysis?.keywords?.join(", ") || "无"}

**扩展关键词**: ${data.analysis?.keywordsExpanded?.join(", ") || "无"}

## 搜索策略

${data.analysis?.searchQueries?.map((q: string, i: number) => `${i + 1}. "${q}"`).join("\n") || "无"}
`;
    } else if (data.topMatches !== undefined) {
      // 搜索结果 -> Markdown
      content = `# Skill Searcher 搜索结果
...
${data.searchQueries?.map((q: string, i: number) => `${i + 1}. "${q}"`).join("\n") || "无"}

## 差距分析

| 匹配度 | 数量 |
|--------|------|
| 高匹配 | ${data.gapAnalysis?.highMatch || 0} |
| 中匹配 | ${data.gapAnalysis?.mediumMatch || 0} |
| 低匹配 | ${data.gapAnalysis?.lowMatch || 0} |

## Top 推荐

| 排名 | 技能 | 匹配度 | 质量 | 建议 |
|------|------|--------|------|------|
${data.topMatches?.map((m: any, i: number) => `| ${i + 1} | ${m.name} | ${m.matchLevel} | ${m.qualityScore} | ${m.suggestions?.[0] || "-"} |`).join("\n") || "无匹配结果"}

## 详细结果

${data.topMatches?.map((m: any) => `### ${m.name}

- **全名**: ${m.fullName}
- **描述**: ${m.description?.slice(0, 100)}...
- **版本**: ${m.version}
- **下载**: ${m.downloads}
- **匹配度**: ${m.matchLevel} (${m.matchScore}%)
- **质量**: ${m.qualityScore}
`).join("\n\n") || "无"}
`;
    } else {
...
function exploreProject(rootDir: string): Partial<ExplorationReport> {
  const items: string[] = [];
  ...
  return {
    ...
    projectType: detectProjectType(rootDir) as any,
...
function parseExplorationReport(jsonStr: string): ExplorationReport | null {
  try {
...
function analyzeIntent(report: ExplorationReport, userIntent: string): AnalysisResult {
  const intentSummary = userIntent || report.userIntent || "探索相关资源";
  const keywords: string[] = [];
  const keywordsExpanded: string[] = [];
...
function parseSearchOutput(output: string): SearchResult[] {
  ...
  for (const line of lines) {
    ...
    if (skillMatch) {
      if (currentItem?.name) results.push(currentItem as SearchResult);
      currentItem = {
...
function calculateMatchScore(result: SearchResult, analysis: AnalysisResult): MatchScore {
  let score = 50;
...
async function installSkill(fullName: string): Promise<boolean> {
  console.log(`\n正在安装: ${fullName}`);
  const proc = Bun.spawn(["42plugin", "install", fullName], {
    stdout: "pipe",
    stderr: "pipe",
  });
...
async function installSkillsBatch(fullNames: string[]): Promise<{ success: string[]; failed: string[] }> {
...
function outputAnalysisReport(analysis: AnalysisResult) {
...
function outputMatchReport(matches: MatchScore[]) {
  console.log("\n" + "=".repeat(60));
...
function outputGapAnalysis(matches: MatchScore[], analysis: AnalysisResult) {
  console.log("=".repeat(60));
...
function generateAdaptationPlan(matches: MatchScore[], analysis: AnalysisResult) {
...
async function main() {
  ...
  let report: ExplorationReport | null = null;
  ...
  if (!report) {
    console.log("Usage: bun workflow.ts [--auto] '<exploration-report-json>' [--search] [--install] [--adapt]");
    console.log("  --auto     自动探索当前项目");
    console.log("  --search   执行搜索");
    console.log("  --install  安装选中技能");
    console.log("  --adapt    生成改写计划");
    console.log("\n示例:");
    console.log("  bun workflow.ts --auto --search");
    console.log('  bun workflow.ts \'{"projectName":"MyProject","projectType":"skill-repository"}\' --search');
    process.exit(1);
  }
  ...
  const explorationReport = {
    ...
    projectDescription: report.projectDescription,
    existingItems: report.existingItems,
    categories: report.categories,
    keyFeatures: report.keyFeatures,
    userIntent: analysis.intentSummary,
    requirements: report.requirements,
    analysis: {
      keywords: analysis.keywords,
      keywordsExpanded: analysis.keywordsExpanded,
      searchQueries: analysis.searchQueries,
    },
  };
  saveReportToFile("skill-searcher-report.md", explorationReport);
  ...
  if (shouldSearch || shouldAdapt || shouldInstall) {
    ...
    const searchResultsReport = {
      ...
      topMatches: sortedMatches.slice(0, 10).map(m => ({
        name: m.result.name,
        fullName: m.result.fullName,
      ...
      gapAnalysis: {
        highMatch: sortedMatches.filter(m => m.matchLevel === "高").length,
        mediumMatch: sortedMatches.filter(m => m.matchLevel === "中").length,
        lowMatch: sortedMatches.filter(m => m.matchLevel === "低").length,
      },
    };
    saveReportToFile("skill-searcher-results.md", searchResultsReport);
